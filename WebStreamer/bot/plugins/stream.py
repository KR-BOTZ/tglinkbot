...
async def private_receive_handler(c: Client, m: Message):
    ...
    if Var.UPDATES_CHANNEL != "None":
        try:
            ...
            if user.status == "kicked":
                await c.send_message(
                    chat_id=m.chat.id,
                    text="__Sᴏʀʀʏ Sɪʀ, Yᴏᴜ ᴀʀᴇ Bᴀɴɴᴇᴅ ᴛᴏ ᴜsᴇ ᴍᴇ.__\n\n  **Cᴏɴᴛᴀᴄᴛ Mʏ [𝗦𝘂𝗽𝗽𝗼𝗿𝘁 𝗚𝗿𝗼𝘂𝗽](https://t.me/kr_join). Tʜᴇʏ Wɪʟʟ Hᴇʟᴘ Yᴏᴜ**",
                    parse_mode="markdown",
    ...
    try:
        ...
        stream_link = "https://watchs-krbotz.onrender.com{}/{}".format(log_msg.message_id, file_name) if Var.ON_HEROKU or Var.NO_PORT else \
            "http://watchs-krbotz.onrender.com/{}/{}".format(log_msg.message_id,
                                    file_name)
...
async def channel_receive_handler(bot, broadcast):
    ...
    try:
        ...
        stream_link = "https://watchs-krbotz.onrender.com/{}".format(log_msg.message_id) if Var.ON_HEROKU or Var.NO_PORT else \
            "http://watchs-krbotz.onrender.com/{}".format(log_msg.message_id)
        ...
        await bot.edit_message_reply_markup(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("Dᴏᴡɴʟᴏᴀᴅ ʟɪɴᴋ 📥", url=f"https://t.me/{(await bot.get_me()).username}?start=MrTamilKiD_{str(log_msg.message_id)}")]])
            # [[InlineKeyboardButton("Dᴏᴡɴʟᴏᴀᴅ ʟɪɴᴋ 📥", url=f"https://t.me/direct_link_generator_658_bot?start=MrTamilKiD_{str(log_msg.message_id)}")]])
        )
